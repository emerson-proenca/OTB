...
COUNTRIES = [
    ...
    "ZAM",
    "ZIM",
]  # ["USA", "FRA"] or [] for all countries

OUTPUT_FILE = "est_fide_tournaments.jsonl"
            ...
            tournaments.append(
                {
                    ...
                    "rcvd": get_text(row[5]),
                    "period": period,
                    # This is not storing LINKs, because they can be easily created with just the FIDE_ID
                    # "link_information": f"https://ratings.fide.com/tournament_information.phtml?event={row[0]}",
                    # "link_event": f"https://ratings.fide.com{a_tag['href']}" if a_tag and "href" in a_tag.attrs else None,
                }
            )
        return tournaments
    except Exception as e:
        ...
        for count, code in enumerate(target_countries, 1):
            ...
            for period in periods:
                ...
                if results:
                    ...
                    total_t = total_t + len(results)
    print(f"Done! Saved {total_t} Tournaments to {OUTPUT_FILE}")
