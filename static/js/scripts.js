        ...
        <div class="card tournament-card" data-i18n="">
            <div class="card-header" data-i18n="">
                <div class="card-title">${
                  tournament.name || "Nome não disponível"
                }</div>
                <div class="tournament-meta" data-i18n="">
                    ${
                      tournament.id
                        ? `<span class="tournament-badge" data-i18n="">ID: ${tournament.id}</span>`
                        : ""
                    }
                    ${
                      tournament.status
                        ? `<span class="tournament-badge status" data-i18n="">${tournament.status}</span>`
                        : ""
                    }
                </div>
            </div>
            <div class="card-body" data-i18n="">
                <div class="tournament-details" data-i18n="">
                    ${
                      tournament.period
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.period">Período:</strong> ${tournament.period}</p>`
                        : ""
                    }
                    ${
                      tournament.location
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.location">Local:</strong> ${tournament.location}</p>`
                        : ""
                    }
                    ${
                      tournament.organizer
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.organizer">Organizador:</strong> ${tournament.organizer}</p>`
                        : ""
                    }
                    ${
                      tournament.time_control
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.timeControl">Ritmo:</strong> ${tournament.time_control}</p>`
                        : ""
                    }
                    ${
                      tournament.total_players
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.totalPlayers">Total de Jogadores:</strong> ${tournament.total_players}</p>`
                        : ""
                    }
                    ${
                      tournament.fide_players
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.fidePlayers">Jogadores FIDE:</strong> ${tournament.fide_players}</p>`
                        : ""
                    }
                    ${
                      tournament.rating
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.rating">Rating:</strong> ${tournament.rating}</p>`
                        : ""
                    }
                    ${
                      tournament.observation
                        ? `<p data-i18n=""><strong data-i18n="js.tournament.observation">Observações:</strong> ${tournament.observation}</p>`
                        : ""
                    }
                    ${
                      tournament.regulation &&
                      tournament.regulation !== "https://www.cbx.org.br"
    ...
    .map(
      (player) => `
        <div class="card player-card" data-i18n="">
            <div class="card-header" data-i18n="">
                <div class="card-title" data-i18n="js.player.error.name">${
            ...
            <div class="card-body" data-i18n="">
                <div class="player-details" data-i18n="">
                    ${
                      player.country
                        ? `<p data-i18n=""><strong data-i18n="js.player.country">País:</strong> ${player.country}</p>`
                        : ""
                    }
                    ${
                      player.state
                        ? `<p data-i18n=""><strong data-i18n="js.player.state">UF:</strong> ${player.state}</p>`
                        : ""
                    }
                    ${
                      player.birthday
                        ? `<p data-i18n=""><strong data-i18n="js.player.birthday">Nascimento:</strong> ${player.birthday}</p>`
                        : ""
                    }
                    ${
                      player.local_id
                        ? `<p data-i18n=""><strong data-i18n="js.player.localId">ID CBX:</strong> ${player.local_id}</p>`
                        : ""
                    }
                    ${
                      player.fide_id
                        ? `<p data-i18n=""><strong data-i18n="js.player.fideId">ID FIDE:</strong> ${player.fide_id}</p>`
                        : ""
                    }
                    ${
                      player.classical
                        ? `<p data-i18n=""><strong data-i18n="js.player.classical">Clássico:</strong> ${player.classical}</p>`
                        : ""
                    }
                    ${
                      player.rapid
                        ? `<p data-i18n=""><strong data-i18n="js.player.rapid">Rápido:</strong> ${player.rapid}</p>`
                        : ""
                    }
                    ${
                      player.blitz
                        ? `<p data-i18n=""><strong data-i18n="js.player.blitz">Blitz:</strong> ${player.blitz}</p>`
                        : ""
                    }
                    ${
                      player.local_profile &&
                      player.local_profile !== "https://www.cbx.org.br/jogador/"
...
function displayNews(news) {
  ...
  const cardsHTML = news
    .map(
      (item) => `
        <div class="card news-card" data-i18n="">
            <div class="card-header" data-i18n="">
                <div class="card-title" data-i18n="js.news.error.name">${
                  item.title || "Título não disponível"
                }</div>
                ${
                  item.date_text
                    ? `<div class="news-date" data-i18n="">${item.date_text}</div>`
                    : ""
                }
            </div>
            <div class="card-body" data-i18n="">
                ${
                  item.link
                    ? `<a href="${item.link}" target="_blank" class="news-link" data-i18n="js.news.link">Ler notícia completa</a>`
                    : ""
                }
            </div>
        </div>
    ...
    .map(
      (item) => `
        <div class="card announcement-card" data-i18n="">
            <div class="card-header" data-i18n="">
                <div class="card-title" data-i18n="js.announcements.error.name">${
                  item.title || "Título não disponível"
                }</div>
                ${
                  item.date_text
                    ? `<div class="announcement-date" data-i18n="">${item.date_text}</div>`
                    : ""
                }
            </div>
            <div class="card-body" data-i18n="">
                ${
                  item.link
                    ? `<a href="${item.link}" target="_blank" class="announcement-link" data-i18n="js.announcements.link">Ler comunicado completo</a>`
                    : ""
                }
            </div>
        </div>
